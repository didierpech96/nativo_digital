   ...
   var $show_count      = true;
	...
	function create_links()
	{
      ...
      if ($this->show_count)
      {
         ...
         $info = '<p>Viendo ' . ( $curr_offset + 1 ) . ' a ' ;

         if( ( $curr_offset + $this->per_page ) < ( $this->total_rows) ) #originalmente se restaba 1
            $info .= $curr_offset + $this->per_page;
         else
            $info .= $this->total_rows;

         $info .= ' de ' . $this->total_rows.'</p>';
	...
	function getAJAXlink( $count, $text) {
        ...
        $params = "{";
        if(!empty($this->additional_param)){
        	foreach ($this->additional_param as $key => $value) {
        		$params .= $key .":"."'".$value."',";
        	}
		...
		return "<a href=\"#\"
		         " . $this->anchor_class . "
					onclick=\"$('#loader-table').show(); $.post('". $this->base_url . $count ."', ". $params .", function(data){
					$('". $this->div . "').html(data); $('#loader-table').hide(); 
            $('". $this->div . "').find('table').basictable('start');" . $this->js_rebind ."; }).done(function(){f_loader_table();});  return false;\">"
				. $text .'</a>';
	}
